    // js 创建canvas元素
const canvas = document.createElement('canvas') as HTMLCanvasElement // 类型断言定义context为非空变量
const tipDom = document.querySelector('.tip') as HTMLElement // 类型断言定义context为非空变量
// 棋盘宽度
const boardWidth: number = 1200
const boardSpace: number = 50
canvas.width = boardWidth
canvas.height = boardWidth
document.body.append(canvas)
// 获取画笔
// const context = canvas.getContext('2d') as CanvasRenderingContext2D // 类型断言定义context为非空变量
const context = canvas.getContext('2d')! // 非空断言定义context为非空变量
// 棋盘横线起点x坐标
const boardStartX = boardSpace
const boardStartY = boardSpace
// 棋盘横/纵线终点x/y坐标
const boardEndX = boardWidth - boardSpace
const boardEndY = boardEndX
// 横向或者纵向棋盘线数量
...
canvas.addEventListener('click', e => {
    ...
    // 限制点击范围
    const xCanClickStart = boardStartX - boardSpace / 2
    const yCanClickStart = xCanClickStart
    const xCanClickEnd  = boardEndX + boardSpace / 2
    const yCanClickEnd = xCanClickEnd
 ...
 * @whiteContinuousNum
 */
function isXWinChecked (xx: number, yy: number): boolean {
 ...
 * @whiteContinuousNum
 */
function isYWinChecked (xx: number, yy: number): boolean {
    let blackContinuousNum = 1
    let whiteContinuousNum = 1
 ...
 * @whiteContinuousNum
 */
function isWs2EnWinChecked (xx: number, yy: number): boolean {
    let blackContinuousNum = 1
    let whiteContinuousNum = 1
 ...
 * @whiteContinuousNum
 */
function isWn2EsYWinChecked (xx: number, yy: number): boolean {
    let blackContinuousNum = 1
    let whiteContinuousNum = 1
 ...
 * @targetArray 数组
 */
function containsArray(twoDimArray: Array<Array<number | boolean>>, targetArray: Array<number | boolean>): boolean {
